...
基于Go语言开发的A股股票买卖点预测系统，支持多种数据源（Tushare、AKTools），提供技术指标计算、基本面数据分析和买卖点预测功能。集成完整的财务报表数据和估值指标，支持技术面与基本面相结合的投资决策分析。
...
![基本面指标](docs/imgs/基本面指标.png)

- **财务报表数据**：利润表、资产负债表、现金流量表
- **估值指标**：市盈率(PE)、市净率(PB)、市销率(PS)
- **智能金额显示**：自动选择万元/亿元单位，数据更直观
- **多期间数据**：支持年报、中报、季报等不同报告期
- **实时更新**：基于最新财务数据和市场行情
- **基本面因子**：价值因子、成长因子、质量因子等量化指标
...
### 🚀 RESTful API
- **技术分析API**：股票日线数据、技术指标、买卖点预测
- **基本面API**：财务报表、估值指标、基本面因子分析
- **综合数据API**：技术面与基本面数据整合接口
- JSON格式数据交换
- CORS支持，便于前端集成
- 详细的错误处理和日志记录
...
- **基本面数据展示**: 财务报表、估值指标、基本面因子分析
- **智能金额格式化**: 自动选择合适单位显示金额（万元/亿元）
- **多维度数据整合**: 技术面与基本面数据统一展示
- **智能搜索**: 支持股票名称和代码实时搜索
