  ...
  reducers: {
    ...
    removeRecentlyViewed: (state, action: PayloadAction<string>) => {
      state.recentlyViewed = state.recentlyViewed.filter(
        item => item.ts_code !== action.payload
      );
    },
    ...
    // 更新热门搜索
    setHotSearches: (state, action: PayloadAction<string[]>) => {
      state.hotSearches = action.payload;
    },
    
    // 更新搜索配置
    updateSearchConfig: (state, action: PayloadAction<Partial<SearchState['searchConfig']>>) => {
...
export const selectRecentlyViewed = (state: { search: SearchState }) => state.search.recentlyViewed;
export const selectHotSearches = (state: { search: SearchState }) => state.search.hotSearches;
export const selectSearchConfig = (state: { search: SearchState }) => state.search.searchConfig;
